...
module.exports = function (grunt) {
  ...
  var task = async function () {
    ...
    // make topline file
    // TODO: figure out where in this process we need to handle published status
    var topline = {
      gcuHeader: grunt.data.archieml.gcu.headline,
      gcuPublished: grunt.data.archieml.gcu.published,
    };
    topline.gcuText = grunt.template.renderMarkdown(
      grunt.data.archieml.gcu.text
    );
    await fs.writeFile("build/data/topline.json", serialize(topline));
    ...
    for (var office in byOffice) {
      await fs.writeFile(
        `build/data/${office}.json`,
        serialize(byOffice[office])
      );
    }
  };
