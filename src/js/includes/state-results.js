...
export class StateResults extends Component {
  constructor(props) {
    ...
    this.state = {currState: props.state.toLowerCase(), activeView: props.activeView};
  ...
  componentWillUnmount() {
    ...
    gopher.unwatch('https://apps.npr.org/elections18-graphics/data/extra_data/' + this.state.currState + '-extra.json', this.onData);
    gopher.unwatch(`https://apps.npr.org/elections18-graphics/data/${this.state.currState}.json`, this.onData);
  }
  ...
  render() {
    ...
    const anyHouseCandidate = this.state.results.house.results[anyHouseRaceID][0];
    ...
    return <div class="results">
              <header id="state-header">
                <div class="state-icon">
                  <i class= {"stateface stateface-" + this.state.currState}>
                  </i>
                </div>
                <h1>
  ...
  renderResults() {
    ...
    if (this.state.activeView === 'key') {
      ...
      return <div>
                {areThereAnyVotesYet ? '' : <p class="poll-closing">{`Last polls close at ${pollCloseTime} ET.`}</p>}
                {Object.keys(data.governor.results).length ? this.renderMiniBigBoard('Governor', 'governor', data.governor.results, 'governor', showCountyResults ? 'County-level results \u203a' : 'Detailed gubernatorial results \u203a') : ''}
                ...
                {Object.keys(data.ballot_measures.results).length ? this.renderMiniBigBoard('Key Ballot Initiatives','ballot-measures', sortBy(getValues(data.ballot_measures.results), race => race[0].seatname.split(' - ')[0])) :' '}
              </div>
      ...
      return <div class="results-house">
              <div class="results-wrapper">
                {sortedHouseKeys.map(race => this.renderRacewideTable(data['house']['results'][race], 'house-race'))}
              </div>
            </div>
  ...
  renderRacewideTable(results, tableClass) {
    ...
    return <div class={tableClass}>
              <table class="results-table">
                ...
                <thead>
                  <tr>
                    ...
                    <th class="amt" scope="col">Votes</th>
                    <th class="amt" scope="col">Percent</th>
                  </tr>
                </thead>
                <tbody>
                ...
                <tfoot>
                  <tr>
                    ...
                    <th class="candidate" scope="row">Total</th>
              ...
              <p class="precincts">
              {this.calculatePrecinctsReporting(results[0]['precinctsreportingpct']) + '% of precincts reporting (' + results[0].precinctsreporting + ' of ' + results[0].precinctstotal + ')'}
              </p>
          </div>
  }
  ...
  renderCandidateName(result) {
    ...
    return <td class="candidate" scope="col">
  ...
  renderMiniBigBoard(title, boardClass, races, linkRaceType, linkText) {
    return <div class={"board " + this.getBoardClasses(boardClass, races)}>
              <h2>
              {title}
              {linkRaceType ? <button name="see-more-nav" data-hook={linkRaceType} onclick={this.switchResultsView.bind(this)}>{linkText}</button> : <span></span>}
              </h2>
              <div class="results-wrapper">
                <div class="results">
                  <div class="column">
                    <table class="races">
                      {Object.values(races).map(race => this.renderRace(race.slice(0, 2)))}
                    </table>
                  </div>
                </div>
              </div>
  ...
  getDataFileName(view) {
    ...
    if (view === 'senate' || view === 'governor') {
      ...
      return 'https://apps.npr.org/elections18-graphics/data/extra_data/' + this.state.currState + '-extra.json';
    } else {
  ...
  renderTabSwitcher() {
    ...
    // TODO: make this real
    const elements = ["key", "house", "governor"].map(tab => this.createTabElement(tab));
    ...
    return  <div class="switcher"> 
              Election results: { delimited}
            </div>
  }

  createTabElement(tab) {
    return <span role="button" onclick={this.switchResultsView.bind(this)} name="race-type-nav" data-hook={tab.toLowerCase()} classes={this.state.activeView === tab.toLowerCase() ? "active" : ""}>
  ...
  renderRace(race) {
    ...
    var firstCand = 
            <Fragment>
              ...
              <th class="state">
                {this.decideLabel(result1)}
              </th>
              <td class={"candidate " + firstCandClasses}>
                <span class="fname"> {result1['first'] ? result1['first'] + ' ' : ''}</span>
                <span class="lname">{results['uncontested'] ? result1['last'] + " (Uncontested)" : result1['last']}</span>
              </td>
              <td class= {"candidate-total " + firstCandClasses}>
                {results['uncontested'] ? '' : <span class="candidate-total-wrapper">{Math.round(result1['votepct'] * 100)}</span>}
              </td>
            </Fragment>

    var secondCandClasses = results['uncontested'] ? '' : createClassesForBoardCells(result2, coloredParties);
    var secondCand =  results['uncontested'] ? (<div></div>) : ( 
            <Fragment>
              <td class="candidate-total-spacer"></td>
              <td class={"candidate-total " + secondCandClasses}>
                {results['uncontested'] ? '' : <span class="candidate-total-wrapper">{Math.round(result2['votepct'] * 100)}</span>}
              </td>
              <td class={"candidate " + secondCandClasses}>
                <span class="fname"> {result2['first'] ? result2['first'] + ' ' : ''}</span>
...
const createClassesForBoardCells = (result, coloredParties, uncontested) => {
  ...
  classes += !coloredParties.includes(result.party) && result.party !== 'Uncontested' ? " other" :"";
