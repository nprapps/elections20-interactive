...
export class BigBoardCore extends Component {
  ...
  render() {
    ...
    return <div class="results-wrapper">
              ...
              <div class="footer">
                <p> Source: AP <span>as of lastupdated ET</span></p>
              </div>
          </div>;
    ...
    if (races) {
      return (
            ...
            <h2 class="bucketed-group">
                <span>{ isBucketedByTime(column['category'])
                    ? column['category'] + ' ET'
                    : column['category'] } 
            ...
            <table class="races">
              <thead class="screen-reader-only">
                <tr>
                  <th scope="col" class="pickup">Pick-up?</th>
                  <th scope="col" class="state">Name</th>
                  <th scope="col" class="candidate">Candidate one name</th>
                  <th scope="col" class="candidate-total">Candidate one vote percent</th>
                  <th scope="col" class="candidate-total-spacer"></th>
                  <th scope="col" class="candidate-total">Candidate two vote percent</th>
                  <th scope="col" class="candidates">Candidate two name</th>
                  <th scope="col" class="results-status">Percent of precincts reporting</th>
                </tr>
              </thead>
  ...
  getTopLevelHeaderGroup(label, party, data, winner) {
    return <div class= {"results-header-group " + party.toLowerCase()}>
              <h2 class="party">
                <label>
    ...
    for (let i = 0; i < sortedCategories.length; i++) {
      ...
      if (raceIndex >= breakingIndex) {
        if (raceIndex > breakingIndex) {
          ...
          var carryOverCategory = JSON.parse(JSON.stringify(sortedCategories[i]));
          carryOverCategory.races = carryOverCategory.races.slice(keep, sortedCategories[i].races.length + 1);
          carryOverCategory.overflow = true;
          sortedCategories.splice(i+1, 0, carryOverCategory);
          sortedCategories[i].races = sortedCategories[i].races.slice(0, keep - 1);
        }
        return {
  ...
  getMetaData(results) {
    ...
    if (called || (result1['votecount'] > 0 || (!results.uncontested && result2['votecount'] > 0))) {
      var reporting = true;
    }
  ...
  determineResults(race) {
    ...
    if (result1.party === result2.party) {
      sortedResults = sortedResults.sort(function (a, b) {
        return sortedResults[0].party === 'GOP' ? a['votepct'] - b['votepct'] : b['votepct'] - a['votepct'];
      });
    }
